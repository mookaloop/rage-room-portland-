...
export default function BookPageClient({ initialLocation }: { initialLocation: LocationSlug }) {
  ...
  return <main className="min-h-screen bg-background text-foreground"><SiteHeader />
    <section className="border-b border-border bg-card py-14 md:py-20"><div className="container mx-auto max-w-6xl px-4 sm:px-6"><p className="font-mono text-sm font-bold uppercase tracking-widest text-secondary">Book online</p><h1 className="mt-3 text-balance font-serif text-5xl font-black uppercase md:text-7xl">Choose your location. <span className="text-primary">Choose your chaos.</span></h1><p className="mt-5 max-w-2xl text-lg leading-relaxed text-muted-foreground">Select St. Johns or Tualatin, then reserve a rage room, axe throwing lane, or combo session.</p></div></section>
    <section className="py-12"><div className="container mx-auto max-w-6xl px-4 sm:px-6"><Tabs value={location} onValueChange={(value) => { setLocation(value as LocationSlug); setSelectedTualatin(null) }}><TabsList className="grid h-auto w-full grid-cols-2"><TabsTrigger value="st-johns" className="py-4 text-base">St. Johns</TabsTrigger><TabsTrigger value="tualatin" className="py-4 text-base">Tualatin</TabsTrigger></TabsList>
      {(["st-johns", "tualatin"] as const).map((slug) => <TabsContent key={slug} value={slug} className="mt-8"><div className="mb-8"><h2 className="font-serif text-3xl font-black uppercase">Book {locations[slug].shortName}</h2><p className="mt-2 flex items-center gap-2 text-muted-foreground"><MapPin aria-hidden="true" />{locations[slug].address}</p></div><div className="grid gap-5 md:grid-cols-3">{locations[slug].bookingLinks.map((offer, index) => <Card key={offer.name} className="flex flex-col bg-card"><CardHeader><CardTitle className="font-serif text-2xl uppercase">{offer.name}</CardTitle><CardDescription>{offer.description}</CardDescription></CardHeader><CardContent className="flex-1"><p className="text-2xl font-black text-secondary">{offer.price}</p><p className="text-sm text-muted-foreground">{offer.duration}</p></CardContent><CardFooter>{slug === "tualatin" ? <Button className="w-full" onClick={() => setSelectedTualatin(index)}>Show available times</Button> : <Button asChild className="w-full"><a href={offer.url} target="_blank" rel="noreferrer">Book on Acuity<ArrowUpRight data-icon="inline-end" /></a></Button>}</CardFooter></Card>)}</div>
